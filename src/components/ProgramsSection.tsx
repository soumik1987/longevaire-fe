...
const ProgramsSection: React.FC = () => {
  ...
  return (
    <div className="programs-section-container">
      ...
      <div className="programs-scroll-wrapper">
        <button className="scroll-arrow left" onClick={() => scroll('left')}>
          <ChevronLeft size={24} />
        </button>
        
        <div className="programs-horizontal-scroll" ref={scrollRef}>
          {programs.map((program) => (
            <div 
              key={program.id} 
              className="programsection-program-card"
              onClick={() => handleProgramClick(program.title)}
            >
              <div className="programsection-program-card-image">
                <img src={program.image} alt={program.title} />
                <div className="programsection-program-card-badge">{program.badge}</div>
