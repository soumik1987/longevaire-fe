// src/pages/dashboard/Facility/ServiceForm.tsx
...
const ServiceForm: React.FC = () => {
  ...
  // In a real app, you would fetch the service data when editing
  useEffect(() => {
    if (isEditing) {
  ...
  const handleSubmit = (action: 'submit' | 'draft' | 'delete') => {
    // Here you would handle form submission based on the action
    console.log('Form action:', action, formData);
    
    if (action === 'delete') {
      if (window.confirm('Are you sure you want to delete this program?')) {
        navigate('/dashboard/facility/services');
      }
      return;
    }
    ...
    <DashboardLayout role="facility">
      <div className="service-form-container">
        <div className="form-header">
          <button className="back-button" onClick={() => navigate('/dashboard/facility/services')}>
            <ArrowLeft size={20} />
          </button>
